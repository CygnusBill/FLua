...
public static class InterpreterLoadFunction
{
    ...
    private static LuaValue[] LoadImplementation(LuaValue[] args)
    {
        ...
        var chunkName = args.Length > 1 && args[1] is LuaString nameStr ? nameStr.Value : "=(load)";
        ...
        try
        {
            ...
            return new LuaValue[] { chunk };
        }
        catch (Exception ex)
        {
            // Return nil and the error message
            return new LuaValue[] { LuaNil.Instance, new LuaString($"{chunkName}: {ex.Message}") };
        }
    }
